...
#if defined(NDEBUG)
# error "Tasoha cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf28865b7f48d603938b21c603b6fe3c19769f9932eb3f225cdc1a5258adafc52");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tasoha: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tasoha Signed Message:\n";
...
{
    // Tasoha: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 60000 blocks
    nSubsidy >>= (nHeight / 60000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tasoha: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tasoha: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1504238413, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "TasohaGroup";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048f8a039d2e67e71c414bf16055ab8bc5baab5d7196d1bb8cf6b0ff1ac65c1f5b8f81747e3e3faad65f20ff2298c15cf0dc4deea29fb87948c5a02e6ce3e6b731") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1504238413;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1510896;
        ...
        assert(block.hashMerkleRoot == uint256("0xd11be2ac3eb36b165b5ca0ae991a97be27aed885b68b88db7a39279ca2c2fcb6"));
...
unsigned char pchMessageStart[4] = { 0x67, 0x0d, 0xfc, 0x69 }; // Tasoha: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TasohaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TasohaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TasohaMiner(CWallet *pwallet)
{
    printf("TasohaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tasoha-miner");
    ...
    try { loop {
        ...
        printf("Running TasohaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TasohaMiner, pwallet));
}
